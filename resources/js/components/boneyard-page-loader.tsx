import { usePageLoading } from '@/hooks/use-page-loading';
import { Skeleton } from 'boneyard-js/react';

/**
 * Global page loading skeleton using Boneyard
 * Shows during Inertia page navigation with auto-captured bones
 */
export default function BoneyardPageLoader() {
    const isLoading = usePageLoading();

    if (!isLoading) return null;

    // Generic page layout that works as fallback
    return (
        <div className="pointer-events-none fixed inset-0 top-0 left-0 right-0 bottom-0 z-40 bg-white/40 dark:bg-slate-950/40">
            <Skeleton name="page-loading" loading={true}>
                <div className="hidden lg:fixed lg:left-64 lg:inset-y-0 lg:right-0 lg:pt-20 lg:px-8 lg:py-8 lg:block">
                    {/* Page Title */}
                    <div className="mb-8">
                        <div className="mb-4 h-8 w-48 rounded bg-slate-100" />
                        <div className="h-4 w-96 rounded bg-slate-50" />
                    </div>

                    {/* Content Grid */}
                    <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                        {[1, 2, 3, 4, 5, 6].map((i) => (
                            <div
                                key={i}
                                className="space-y-4 rounded-lg border border-slate-200 bg-white p-6 dark:border-slate-800 dark:bg-slate-900"
                            >
                                <div className="h-6 w-3/4 rounded bg-slate-100" />
                                <div className="space-y-2">
                                    <div className="h-4 w-full rounded bg-slate-50" />
                                    <div className="h-4 w-5/6 rounded bg-slate-50" />
                                    <div className="h-4 w-4/6 rounded bg-slate-50" />
                                </div>
                            </div>
                        ))}
                    </div>
                </div>
            </Skeleton>
        </div>
    );
}
